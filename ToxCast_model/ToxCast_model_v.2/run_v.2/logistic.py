...
def main(fingerprint_type, file_path, model_save_path, assay_num, fp_path, time_now, data_name):
    ...
    # 파일 이름 포맷
    prefix = f"{time_now}_{fingerprint_type}_logistic"
...
############################################################################04.07 아래 수정 x
    ...
    for p in tqdm(range(len(params))):
        ...
        for train_idx, val_idx in kf.split(x_train, y_train):
            ...
            model = LogisticRegression(random_state=random_seed, **params[p])
    ...
    final_model = LogisticRegression(random_state=random_seed, **best_model)
    ...
    # 최적 모델 저장
    model_filename = f'{model_save_path}/{assay_name}_best_model_{fingerprint_type}_logistic.joblib'
